    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initData(JsonData, new OnInitDataFinishedListener() {
            ...
            public void OnInitDataListener(boolean success) {
                Log.d("jht", "OnInitDataListener: "+success);
...
//                        selfDialog.dismiss();
//                        SuoPingDialog = new SuoPingDialog(HistoryTemDataActivity.this,"加载中......");
//                        SuoPingDialog.show();
//                        //提交日期操作从日历获得
    ...
    public void initData(String jsonData, final OnInitDataFinishedListener listener){
        ...
        OkHttpUtilTWO.PostNewDownRawTWO("mic-service-data/down?micServiceId=" + deviceType + "&timeOut=30", jsonData, new OkHttpUtilTWO.ReqCallBack() {
            ...
            public void onReqSuccess(Object result) throws JSONException {
                ...
                try {
                    historyBean = gson.fromJson((String) result,HistoryBean.class);
                    Log.d("zyq", "onReqSuccess  thisthishtisthist: "+result.toString());
                } catch (JsonSyntaxException e) {
                    e.printStackTrace();
                    util.showToast(HistoryTemDataActivity.this,"产品解析出错");
                }
                Log.d("zyq", "getData  thisthishtisthist: "+historyBean.getData().size());
                if (historyBean.getData()!=null && historyBean.getData().size()>0) {
