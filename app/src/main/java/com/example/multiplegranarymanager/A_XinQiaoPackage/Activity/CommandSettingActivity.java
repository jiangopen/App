...
import com.example.multiplegranarymanager.A_XinQiaoPackage.Body.UpDateCommand;
    ...
    public void onClick(View v) {
        switch(v.getId()){
            case R.id.txt_submit:
                ...
                // 获取温度组的EditText文本内容并转换为整数设置给对应属性
                String temZuText = String.valueOf(et_tem_zu.getText());
                if (temZuText.isEmpty()) {
                    Toast.makeText(this, "温度组不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setZu(Integer.valueOf(temZuText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "温度组输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }

// 获取温度层的EditText文本内容并转换为整数设置给对应属性
                String temLanText = String.valueOf(et_tem_lan.getText());
                if (temLanText.isEmpty()) {
                    Toast.makeText(this, "温度层不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setLan(Integer.valueOf(temLanText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "温度层输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }

// 获取温度仓的EditText文本内容并转换为整数设置给对应属性
                String temCengText = String.valueOf(et_tem_ceng.getText());
                if (temCengText.isEmpty()) {
                    Toast.makeText(this, "温度仓不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setCeng(Integer.valueOf(temCengText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "温度仓输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }

// 获取湿度组的EditText文本内容并转换为整数设置给对应属性
                String mouZuText = String.valueOf(et_mou_zu.getText());
                if (mouZuText.isEmpty()) {
                    Toast.makeText(this, "湿度组不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setMoistureZu(Integer.valueOf(mouZuText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "湿度组输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }

// 获取湿度层的EditText文本内容并转换为整数设置给对应属性
                String mouLanText = String.valueOf(et_mou_lan.getText());
                if (mouLanText.isEmpty()) {
                    Toast.makeText(this, "湿度层不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setMoistureLan(Integer.valueOf(mouLanText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "湿度层输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }

// 获取湿度仓的EditText文本内容并转换为整数设置给对应属性
                String mouCengText = String.valueOf(et_mou_ceng.getText());
                if (mouCengText.isEmpty()) {
                    Toast.makeText(this, "湿度仓不能为空，请输入有效数字", Toast.LENGTH_SHORT).show();
                    return;
                }
                try {
                    CommandMapData.getGranarySetting().setMoistureCeng(Integer.valueOf(mouCengText));
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "湿度仓输入格式不正确，请输入有效数字", Toast.LENGTH_SHORT).show();
                }
...
//                CommandMapData.getGranarySetting().setMoistureCeng(Integer.valueOf(String.valueOf(et_mou_ceng.getText())));
                ...
                upDate.setData(BODY.getData());
                upDate.setCommandMapId(commandMapId);
                upDate.setVersion(BODY.getVersion());
                NewDownRawBodyTWO Body = new NewDownRawBodyTWO();
                Body.setQuery(null);
                Body.setMethod("PUT");
                Body.setHeaders(null);
                Body.setUrl("/command-map");
                Body.setBody(upDate);
                String json = gson.toJson(Body);
